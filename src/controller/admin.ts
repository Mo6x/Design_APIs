...
export async function createGroup(req: Request, res: Response) {
  ...
  try {
    ...
    await pool.query('UPDATE admin SET group_id = $1 WHERE email = $2', [groupId, email]);
...
export async function addMembers(req: Request, res: Response) {
    ...
    try {
      ...
      for (const member of members) {
        await pool.query('INSERT INTO group_members (group_id, member_email, status) VALUES ($1, $2, $3)', [groupId, member, 'pending']);
      }
  ...
  export async function updateMemberStatus(req: Request, res: Response) {
    ...
    try {
      ...
      await pool.query('UPDATE group_members SET status = $1 WHEREgroup_id = $2 AND id = $3', [status, groupId, memberId]);
